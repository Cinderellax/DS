...
int Length(L)//求表长,并返回
position LocateElem(L,e)//查找e所在位置并返回
ELemType Getelem(L,i)//获取在第i个位置的值
void ListInsert(&L,i,e)//在第i个位置上插入e
void ListDelete(&L,i,&e)//删除第i个元素，并用e返回
void PrintList(L)//输出所有元素
int Empty(L)//判断是否为空并返回
...
int StackEmpty(s)	//判断栈是否为空栈，并返回标志
void push(&s,x)	//入栈一个元素值为x的元素
void pop(&s,&x)	//出栈一个元素并存储到x中 
void GetTop(s,&x)//求栈顶元素的值并存入x
void ClearStack(&s)//销毁栈
```

#### 队列

```c
void InitQueue(&q)//初始化队列，构造一个空的
int QueueEmpty(q)//判断队列是否为空
void EnQueue(&q,x)//入队
void DeQueue(&q,&x)//出队并存入x
void GetHead(q,&x)//读队头元素，赋值给x
```

#### 图

```c
int Adjacent(G,x,y)//判断是否存在边（x,y）或<x,y>,并返回标志
Set Neighbors(G,x)//列出所有与x邻接的边
void InsertVertex(G,x)//插入顶点x
void DeleteVertex(G,x)//删除顶点x
void AddEdge(G,x,y)//插入边(x,y)或<x,y>
void RemoveEdge(G,x,y)//删除边(x,y)或<x,y>
int FirstNeighbor(G,x)//求图中x第一个领接点，有则返回顶点号，没有则Null
int NextNeighbor(G,x,y)//假设y为x的一个邻接点，返回除y外的下一个领接的顶点号（如果有的话）
int Get_edge_value(G,x,y)//获得边的权值
void Set_edge_value(G,x,y,v)//设置边的权值为v
```
...
​			思路2：双指针，找到中间节点，将中间节点后面的节点反转，从变形链表的两头向中间靠近，依次对比节点，两指针相遇则成立。（若为双向则不用反转）
...
​	4.递归算法求两个正整数m和n的最大公因数。（同时用非递归算法写出）
...
int gcd(int m,int n)//非递归算法使用栈
{
    int top,t,s[30][2];//s为栈
    top=1;
    s[top][0]=m;
    s[top][1]=n;
    while(s[top][1]!=0)
...
​	3.所有分支节点度数为2的二叉树称为正则树，写一个算法判断一颗树是否是正则树。（递归）
...
​	7.写出在中序线索二叉树里查找指定结点在后序的前驱结点的算法。
...
​	13.后序线索二叉树的给定结点的前驱后继，给定结点node,前驱或者后继储存到x
...
​	4.元素集合已存入整型数组A[1..n]中，试写出依次取A中各值A[i](1<=i<=n)构造一棵二叉排序树T的非递归算法
...
void  Creat_BST（BiTree bst，datatype  K[]，int n）
        ...
        {for（i=1；i<=n；i++）
          {p=bst；f=null；//在调用Creat_BST时，bst=null
           while（p!=null）
              if（p->data<K[i]）{f=p；p=p->RLINK； } // f是p的双亲
             else if（p->data>K[i]）{ f=p；p=p->LLINK；}
         s=（BiTree）malloc（sizeof （BiNode））；// 申请结点空间
         s->data=K[i]；s->LLINK=null；s->RLINK=null；
         if（f==null）bst=s；                  //根结点
         else if（s->data<f->data）f->LLINK=s；//左子女
                else  f->RLINK=s；//右子树根结点的值大于等于根结点的值
        }//算法结束
```

​	5.二叉排序树采用二叉链表存储。写一个算法，删除结点值是X的结点。要求删除该结点后,此树仍然是一棵二叉排序树，并且高度没有增长
...
​	6.根到叶子节点的最大距离被称为树的半径 给定一个无向连通图 写一个算法以找出半径最小的生成树
...
int MiniRadius(AdjList g,int v){
    ...
    node R,Q[];
    R.v=v;R.level=1;
    InitQueue(Q);EnQueue(Q,R);
    while(!Empty(Q))
    {
        R=DeQueue(Q);
        v=R.v;l=R.level;p=g[v].firstarc;flag=0;
        while(p){
            w=p->adjvex;
            if(visited[w]==0){
            flag=1;R.level=l+1;EnQueue(Q,R);
            }
            p=p->nextarc;
        }
        if(flag==0)
        ...
        for(i=1;i<=n;i++)
            for(j=1;j<=n;j++)
                if(w[i][j]>w[i][k]+w[k][j])
                    w[i][j]=w[i][k]+w[k][j];
    v=1;
    dist=MAXINT; //中心点初值顶点 v 为１，偏心度初值为机器最大数。
    for(j=1;j<=n;j++)
    {s=0;
     for(i=1;i<=n;i++)
         if(w[i][j]>s)
...
​	2.设G是一个用邻接表表示的连通无向图。对于G中某个顶点v，若从G中删去顶点v及与顶点v相关联的边后，G变成由两个或两个以上非空连通分量所组成的图，则称v是原来图G的一个关节顶点。求出该图的关节点。

​	解由关节点定义及特性可知，若生成树的根有多于或等于两棵子树，则根结点是关节点；若生成树某非叶子顶点 v，其子树中的结点没有指向 v 的祖先的回边，则 v 是关节点。因此，对图 G＝(v,{Edge})，将访问函数 visited[v]定义为深度优先遍历连通图时访问顶点 v 的次序号，并定义一个 low( )函数：
​							low(v)＝Min(visited[v],low[w],visited[k])
其中(v,w)∈Edge，(v,k)∈Edge，w 是 v 在深度优先生成树上的孩子结点，k 是 v 在深度优先生成树上的祖先结点。在如上定义下，若 low[w]>=visited[v]，则 v 是根结点，因 w 及其子孙无指向 v 的祖先的回边。由此，一次深度优先遍历连通图，就可求得所有关节点：
...
{
    for (vi=1;vi<=n;vi++) 
        visited[vi]=0; //图有 n 个顶点，访问数组初始化。
    count=1; 
    visited[1]=1; //设邻接表上第一个顶点是生成树的根。
    p=g[1].firstarc;
    v=p->adjvex;
    dfs_articul(v);
...
```c
void check(int list[]) //list[]为AOV网顶点的某个排列
{
  static int w=1; //拓扑排序序列的计数
  int i,j;
  for(i=0;i<n;i++)
  {
   j=list[i];
    if(count[j]==0)  //count[]存储一开始个节点的入度情况
      {if（i==(n-1)）
          return 1;//到最后一个则是拓扑
      compute(j);//  compute(j) 结点j的后继结点入度减一      
       } 
    else break;  
   }
}
