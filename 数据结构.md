...
​	**度**：与**该顶点**相关的**边的条数**	$\sum _{i=1}^n TD(V_i)=2e$
...
```c
typedef struct ArcNode{
    int ivex;                 //ivex jvex 分别为该边依附的两个顶点
    struct ArcNode *ilink;    //ilink jlink分别为两个顶点所依附的边
    int jvex;
...
算法时间复杂度为**O(n+e)**       以**邻接矩阵**方式实现时 复杂度为O(n^2)； 
...
普里姆算法时间复杂度为O(n^2)    故其复杂度只与顶点有关系 所以该算法适用于**稠密图**

​          (数据结构中对于稀疏图的定义为：有很少条边或弧（边的条数|E|远小于|V|²）且分布无规律的图称为稀疏       				图，反之边的条数|E|接近|V|²，称为稠密图)
...
科鲁斯卡算法的时间复杂度一般取决于sort()函数的复杂度 其复杂度一般大于常量级

主要由边的数量所决定 与顶点数无关 因此适用于**稀疏图**

**什么样的图的最小生成树是唯一的？**

​	所有权值均不相等，或者有相等的边，但是在构造最小生成树的过程中权值相等的边都被并入生成树的图
...
时间复杂度为O(n^2);   该算法用邻接表实现时 时间复杂度为O(n+e);

 该算法是求**某一**顶点到其余各顶点的最短路径
...
时间复杂度为O(n^3);    该算法是求任意顶点到其余各顶点的最短路径
...
重点掌握手工求关键路径的过程 （至于算法实现 我的天 河海应该不那麽变态吧）
...
1.若无向图有7个顶点 要保证图G在**任何情况下**都是**连通**的，则需要的边数**最少**是多少？

（要求边数最少 所以并不是求对应顶点数完全无向图的边数）

要保证N个顶点的无向图在任何情况下都是连通的 则需要先由n-1个顶点组成完全无向图，再从第N个顶点引一条到n-1个任意顶点的边 则图肯定是连通的       故该题为6(6-1)/2 = 15 15+1=16;

2.若一个有向图具有**拓扑排序序列** 那么他的邻接矩阵必定为**一般矩阵** 

3.若一个有向图具有**拓扑有序排序序列** 那么他的邻接矩阵必定为**一般矩阵**         //分别出自1800 和 王道 我不是很 //理解**拓扑有序排序序列** 和 **拓扑有序排序序列** 到底什么区别。。。。
