...
​	有向完全图：若有向图有n个顶点，则最多具有**n(n-1)**条边，则具有**n(n-1)**条边的有向图叫做有向完全图

​	强连通图：在**有向图**中，对于图中的**任意一对顶点**Vi Vj,从Vi到Vj和Vj到Vi都有路径，则该图为强连通图

​			   否则， 其中的**极大强连通子图**称为强连通分量
...
​	无向完全图：若有向图有n个顶点，则最多具有**n(n-1)/2**条边，则具有**n(n-1)/2**条边的有向图叫做无向完全图

​	连通图：在**无向图**中，图中任意两个顶点都连通，则称该图为连通图

​			否则，图中的**极大连通子图**称为连通分量。



路径和路径长度：路径为**相邻顶点序偶**所构成的序列 路径长度是指路径上边的数目
